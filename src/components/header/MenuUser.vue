...
<template>
    <div v-if="isOpenMenuUser" :class="[isAnimarUser ? 'translate-x-0 opacity-100' : 'translate-x-96 opacity-0']"
        class="hidden md:flex fixed top-0 bottom-0 left-0 right-0 transition-all z-50">
        <button @click="$emit('handle-click-menu-user')" type="button"
        ...
        <div @click="$emit('handle-click-menu-user')" class="flex-1"></div>

        <div class="bg-emerald-700 w-1/3">
            <nav class="px-4 mt-8 flex flex-col text-teal-100">
                <RouterLink :to="{ name: 'user.perfil', params: { user: authStore.user.usuario } }" class="mb-6"
